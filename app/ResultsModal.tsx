...
// ─── Nudge copy per classification ───────────────────────────────────────────
const NUDGE_COPY: Record<Classification, string> = {
  "Cultural":         "Bet your friends think this is in the Bible. Prove them wrong 👇",
  "Church Tradition": "Most people assume this is Scripture. Surprised? They will be too 👇",
  "Inferred":         "The answer might surprise your friends. Share the verdict 👇",
  "Concept Present":  "Know someone debating this? Share the facts 👇",
  "Directly Stated":  "Share this verse with someone who needs it 👇",
};
...
function ShareMenu({
  ...
  return (
    ...
    >
      {/* ── Post preview ── */}
      <div style={{ background: T.parchment, borderRadius: 10, padding: "10px 12px", marginBottom: 10, border: `1px dashed ${T.inkFt}` }}>
        <div style={{ fontFamily: T.mono, fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: T.inkLt, marginBottom: 6 }}>Post Preview</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
          <span style={{ fontSize: 13 }}>{badge.icon}</span>
          <span style={{ fontFamily: T.sans, fontSize: 12, fontWeight: 600, color: T.ink }}>Is &ldquo;{result.query}&rdquo; in the Bible?</span>
        </div>
        ...
        <div style={{ fontFamily: T.mono, fontSize: 9.5, color: T.blue, marginTop: 6 }}>{SITE_URL.replace("https://", "")}</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginBottom: 8 }}>
        {socials.map((s) => (
          <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px 8px", borderRadius: 9, textDecoration: "none", background: s.bg, color: s.color, fontFamily: T.sans, fontSize: 12, fontWeight: 600, border: "1px solid transparent", transition: "filter .12s" }}
      ...
      <button onClick={handleCopy}
        style={{ width: "100%", padding: "10px 14px", borderRadius: 9, border: "none", background: copied ? T.greenLt : T.blueLt, color: copied ? T.green : T.blue, fontFamily: T.sans, fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background .2s, color .2s" }}
      >
        {copied
...
export default function ResultsModal({ result, onClose, onSearch }: ResultsModalProps) {
  ...
  const nudge = NUDGE_COPY[result.classification] ?? NUDGE_COPY["Cultural"];
    ...
    <>
      <style>{`
        ...
        @media (max-width:600px) {
          ...
          .share-nudge      { display:none !important; }
        }
      ...
      >
        <div className="modal-shell" style={{ width:"100%", maxWidth:760, maxHeight:"92dvh", background:T.parchment, borderRadius:20, overflow:"hidden", display:"flex", flexDirection:"column", boxShadow:"0 32px 80px rgba(26,22,18,.4), 0 0 0 1px rgba(26,22,18,.08)", animation:"modalIn .28s cubic-bezier(.4,0,.2,1)" }}>
          ...
          {/* ── Share nudge banner ── */}
          <div
            className="share-nudge"
            style={{
              padding: "10px 20px",
              background: `linear-gradient(135deg, ${T.blueLt} 0%, #E8F0FC 100%)`,
              borderTop: `1px solid #C0D4F0`,
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              gap: 12,
              flexShrink: 0,
              animation: "nudgeIn .4s ease .3s both",
            }}
          >
            <p style={{
              fontFamily: T.sans,
            ...
            <button
              onClick={() => {
                const el = document.querySelector(".share-trigger") as HTMLElement;
                el?.click();
              }}
              style={{
                flexShrink: 0,
