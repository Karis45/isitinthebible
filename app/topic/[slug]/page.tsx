...
const TOPICS: Topic[] = [
  { slug: "god-helps-those-who-help-themselves",  query: "God helps those who help themselves"   },
  { slug: "cleanliness-is-next-to-godliness",     query: "Cleanliness is next to godliness"      },
  { slug: "this-too-shall-pass",                  query: "This too shall pass"                   },
  { slug: "spare-the-rod-spoil-the-child",        query: "Spare the rod spoil the child"         },
  { slug: "money-is-the-root-of-all-evil",        query: "Money is the root of all evil"         },
  { slug: "pride-comes-before-a-fall",            query: "Pride comes before a fall"             },
  { slug: "to-thine-own-self-be-true",            query: "To thine own self be true"             },
  { slug: "lion-shall-lie-down-with-the-lamb",    query: "The lion shall lie down with the lamb" },
  { slug: "forbidden-fruit",                      query: "Forbidden fruit"                       },
  { slug: "the-rapture",                          query: "The Rapture"                           },
  { slug: "purgatory",                            query: "Purgatory"                             },
  { slug: "the-trinity",                          query: "The Trinity"                           },
  { slug: "original-sin",                         query: "Original Sin"                          },
  { slug: "guardian-angels",                      query: "Guardian Angels"                       },
  { slug: "soul-sleep",                           query: "Soul Sleep"                            },
  { slug: "free-will",                            query: "Free Will"                             },
  { slug: "prosperity-gospel",                    query: "Prosperity Gospel"                     },
  { slug: "predestination",                       query: "Predestination"                        },
  { slug: "baptism",                              query: "Baptism"                               },
  { slug: "confession",                           query: "Confession"                            },
  { slug: "transubstantiation",                   query: "Transubstantiation"                    },
  { slug: "immaculate-conception",                query: "Immaculate Conception"                 },
  { slug: "penance",                              query: "Penance"                               },
  { slug: "manifesting",                          query: "Manifesting"                           },
  { slug: "karma",                                query: "Karma"                                 },
  { slug: "reincarnation",                        query: "Reincarnation"                         },
  { slug: "eye-of-a-needle",                      query: "Eye of a needle"                       },
  { slug: "turn-the-other-cheek",                 query: "Turn the other cheek"                  },
  { slug: "love-thy-neighbor",                    query: "Love thy neighbor"                     },
  { slug: "judge-not-lest-ye-be-judged",          query: "Judge not lest ye be judged"           },
  { slug: "the-golden-rule",                      query: "The Golden Rule"                       },
  { slug: "do-unto-others",                       query: "Do unto others"                        },
  { slug: "prodigal-son",                         query: "Prodigal son"                          },
  { slug: "does-god-hate-sinners",                query: "Does God hate sinners"                 },
  { slug: "is-suicide-unforgivable",              query: "Is suicide unforgivable"               },
  { slug: "can-women-be-pastors",                 query: "Can women be pastors"                  },
  { slug: "is-abortion-in-the-bible",             query: "Is abortion in the Bible"              },
  { slug: "homosexuality-in-the-bible",           query: "Homosexuality in the Bible"            },
  { slug: "is-tattoos-a-sin",                     query: "Is tattoos a sin"                      },
  { slug: "is-drinking-alcohol-a-sin",            query: "Is drinking alcohol a sin"             },
  { slug: "hell",                                 query: "Hell"                                  },
  { slug: "heaven",                               query: "Heaven"                                },
  { slug: "armageddon",                           query: "Armageddon"                            },
  { slug: "666-mark-of-the-beast",                query: "666 Mark of the Beast"                 },
  { slug: "second-coming",                        query: "Second Coming"                         },
  { slug: "resurrection",                         query: "Resurrection"                          },
  { slug: "judgment-day",                         query: "Judgment Day"                          },
  { slug: "eternal-life",                         query: "Eternal life"                          },
];
...
// ─── JSON extractor — handles thinking blocks and markdown fences ─────────────
function extractJSON(raw: string): string {
  ...
  if (end === -1) throw new SyntaxError("Incomplete JSON object");
...
// ─── Static params — empty so no AI calls at build time ──────────────────────
...
export async function generateMetadata(
  ...
  const description = `Find out if "${topic.query}" actually appears in Scripture — with the exact verses, historical context, and scholarly analysis.`;
  ...
  return {
    ...
    openGraph: { title, description, url: pageUrl, type: "article", images: [{ url: ogUrl, width: 1200, height: 630, alt: title }] },
...
export default async function TopicPage(
  ...
  const jsonLd = {
    ...
    mainEntity: {
      ...
      acceptedAnswer: { "@type": "Answer", text: `${result.oneLiner} ${result.misquoteReality}` },
    },
  };
      ...
      <div style={{ background: "#F5F1E8", minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #1A3A6A 0%, #0F2347 100%)", padding: "48px 24px 40px" }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,.7)", lineHeight: 1.65, maxWidth: 580, marginBottom: 28 }}>{result.oneLiner}</p>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px" }}>
          ...
          {result.verses.length > 0 && (
            <section style={{ marginBottom: 32 }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 26, fontWeight: 400, color: "#1A1612", marginBottom: 16, letterSpacing: "-.3px" }}>Relevant Scripture</h2>
          ...
          {/* Analysis */}
          <section style={{ marginBottom: 32 }}>
            <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 26, fontWeight: 400, color: "#1A1612", marginBottom: 16, letterSpacing: "-.3px" }}>Scholarly Analysis</h2>
          ...
          {result.timeline.length > 0 && (
            <section style={{ marginBottom: 32 }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 26, fontWeight: 400, color: "#1A1612", marginBottom: 20, letterSpacing: "-.3px" }}>Historical Timeline</h2>
          ...
          {result.relatedTopics.length > 0 && (
            <section style={{ marginBottom: 40 }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 22, fontWeight: 400, color: "#1A1612", marginBottom: 14 }}>Related Topics</h2>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {result.relatedTopics.map((t) => {
                  ...
                  return (
                    <a key={t.query} href={`/topic/${relSlug}`} style={{ padding: "9px 14px", border: "1px solid #D8D0C4", borderRadius: 100, background: "#F5F1E8", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ textAlign: "center", padding: "40px 24px", background: "linear-gradient(135deg, #1A3A6A 0%, #0F2347 100%)", borderRadius: 20 }}>
            <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 28, fontWeight: 300, color: "white", marginBottom: 10, letterSpacing: "-.5px" }}>Ask your own question</h2>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,.65)", marginBottom: 22, lineHeight: 1.6 }}>Check any phrase, doctrine, or belief against all 31,102 Bible verses — free, instant, no account needed.</p>
            <a href="/" style={{ display: "inline-block", padding: "12px 28px", background: "white", color: "#1A3A6A", fontSize: 14, fontWeight: 600, textDecoration: "none", borderRadius: 12 }}>Search the Bible →</a>
          </div>
