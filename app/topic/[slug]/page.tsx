...
// ─── Slug ↔ cache key helpers ─────────────────────────────────────────────────
// Must match the cacheKey() function in route.ts exactly
...
// ─── Classification colours (no Tailwind — matches your design tokens) ────────
const BADGE: Record<Classification, { bg: string; text: string; border: string; dot: string; label: string }> = {
  "Directly Stated":  { bg: "#EBF5EF", text: "#1A5C38", border: "#A8D4B8", dot: "#1A5C38", label: "Directly Stated"  },
  "Concept Present":  { bg: "#F5ECD2", text: "#7A5A00", border: "#D4B870", dot: "#B8860B", label: "Concept Present"  },
  "Inferred":         { bg: "#FEF0E6", text: "#8A3A00", border: "#E8C4A0", dot: "#8A3A00", label: "Inferred"         },
  "Cultural":         { bg: "#FEF0F0", text: "#7A1A1A", border: "#E8BEBE", dot: "#7A1A1A", label: "Not in the Bible" },
  "Church Tradition": { bg: "#F3EEF8", text: "#4A1A7A", border: "#C8A8E8", dot: "#4A1A7A", label: "Church Tradition" },
};

const scoreLabel = (s: number) =>
  ["", "Not in the Bible", "Church Tradition", "Inferred", "Concept Present", "Directly Stated"][s] ?? "";
...
function buildJsonLd(result: BibleResult, url: string) {
  return {
    ...
    "@graph": [
      // QAPage — picked up by Google AI Overviews + Perplexity
      {
        "@type":      "QAPage",
        "@id":        `${url}#qapage`,
        url,
      ...
      // Article — gives AI crawlers a clean document to cite
      {
        "@type":         "Article",
        "@id":           `${url}#article`,
        url,
      ...
      // FAQPage from related topics — more AI surface area
      {
        "@type":      "FAQPage",
        "@id":        `${url}#faq`,
  ...
  return (
    <>
      ...
      <div style={{ background: "#F5F1E8", minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        {/* ── Nav ── */}
        <nav style={{ background: "white", borderBottom: "1px solid #D8D0C4", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <header style={{ background: "linear-gradient(135deg, #1A3A6A 0%, #0F2347 100%)", padding: "48px 24px 40px", marginTop: 16 }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            ...
            {/* H1 — the exact question AI engines will try to answer */}
            ...
            {/* One-liner — the citable answer AI engines quote */}
            ...
            <div style={{ maxWidth: 400 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: "rgba(255,255,255,.4)" }}>Biblical Explicitness</span>
              ...
              <div style={{ height: 6, borderRadius: 100, background: "rgba(255,255,255,.15)" }}>
                <div style={{ height: "100%", width: `${((result.explicitnessScore - 1) / 4) * 88 + 6}%`, background: result.explicitnessScore >= 4 ? "#5CC88A" : result.explicitnessScore === 3 ? "#F0C040" : "#E88080", borderRadius: 100, transition: "width .6s" }} />
              </div>
            </div>
          </div>
        </header>
        ...
        <main style={{ maxWidth: 760, margin: "0 auto", padding: "32px 24px 64px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 32 }}>
            <div style={{ padding: "16px 18px", borderRadius: 14, background: "#FEF0F0", border: "1px solid #E8BEBE" }}>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: "#7A1A1A", marginBottom: 8 }}>❌ Common Claim</div>
            ...
            <div style={{ padding: "16px 18px", borderRadius: 14, background: "#EBF5EF", border: "1px solid #A8D4B8" }}>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: "#1A5C38", marginBottom: 8 }}>✓ What Scripture Says</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, marginBottom: 32 }}>
            {[
              ...
              <div key={s.label} style={{ padding: "12px 14px", borderRadius: 12, background: "white", border: "1px solid #D8D0C4" }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: "#8A7D72", marginBottom: 4 }}>{s.label}</div>
                <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 15, fontWeight: 600, color: "#1A1612" }}>{s.value}</div>
              </div>
            ))}
          </div>

          {/* ── Bible Verses — the most citable content for AI ── */}
              ...
              {result.verses.map((v, i) => (
                <article key={v.ref} style={{ padding: "18px 20px", borderRadius: 14, background: "white", border: "1px solid #D8D0C4" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    <span style={{ width: 22, height: 22, borderRadius: 6, background: "#1A3A6A", color: "white", fontFamily: "'DM Mono', monospace", fontSize: 9, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{i + 1}</span>
                  ...
                  {/* blockquote is highly citable by AI engines */}
                  <blockquote style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 16, lineHeight: 1.75, color: "#1A1612", fontStyle: "italic", marginBottom: 10, borderLeft: "3px solid #D8D0C4", paddingLeft: 14 }}>
                    &ldquo;{v.text}&rdquo;
                  </blockquote>
            ...
            {result.timeline.map((node, i) => (
              <div key={node.year} style={{ display: "flex", gap: 16 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, width: 20 }}>
                  <div style={{ width: 12, height: 12, borderRadius: "50%", marginTop: 4, flexShrink: 0, background: i === 0 ? "#1A3A6A" : "white", border: `2px solid ${i === 0 ? "#1A3A6A" : "#D8D0C4"}` }} />
                  {i < result.timeline.length - 1 && <div style={{ width: 1, flex: 1, background: "#D8D0C4", minHeight: 28 }} />}
                </div>
                <div style={{ paddingBottom: 24 }}>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 600, color: "#1A3A6A", marginBottom: 3 }}>{node.year} — {node.label}</div>
          ...
          {/* ── Related topics — internal links boost both SEO and AEO ── */}
              ...
              {result.relatedTopics.map((t) => {
                ...
                return (
                  <Link
                    key={t.query}
                    href={`/topic/${rSlug}`}
                    style={{ padding: "9px 14px", border: "1px solid #D8D0C4", borderRadius: 100, background: "white", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}
                  >
          ...
          {/* ── CTA ── */}
          <div style={{ marginTop: 48, padding: "32px 28px", borderRadius: 20, background: "linear-gradient(135deg, #1A3A6A 0%, #0F2347 100%)", textAlign: "center" }}>
